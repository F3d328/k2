...
public class Login extends HttpServlet {
	    ...
	    try {
	        ...
	        while (rs.next()) {
	            if (email.compareTo(rs.getString(1)) == 0) {
	                if (checkPsw(password, rs.getString(2))) {
	                    control = true;
	...
	// Funzione per controllare la password con SHA-256
	private boolean checkPsw(String password, String storedHash) {
	    try {
	        // Creazione dell'oggetto MessageDigest per SHA-256
	        ...
	        // Confronta l'hash calcolato con quello memorizzato nel database
